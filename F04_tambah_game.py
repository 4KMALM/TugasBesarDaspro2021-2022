...
def last_game_id(arr):
    ...
    if last_game_index < 9:
        return "GAME00" + str(last_game_index)
    elif 9 <= last_game_index < 99:
        return "GAME0" + str(last_game_index)
    else:
        return "GAME" + str(last_game_index)
...
def tambah_game(arr):
    ...
    while salah is True:
        ...
        new_game_data = [input(f"masukan {i}: ") for i in q_list]
        ...
        if invalid != 0:
            ...
            game_data += [[last_game_id(game_data)] + new_game_data]
            arr[0] = game_data
            arr[1] = arr[1] + 1
